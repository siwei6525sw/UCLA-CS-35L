...
/* the following global variables need to be read by all threads,
   but they are only written to before the threads are created */

/* number of threads, to iterate */
int nthreads;

/* scene, for every thread to trace */
scene_t scene;

/* scaled colors to print after pthreads joined */
float scaled_color[width][height][3];

void *
trace_pixels(void *thread_num)
{
    ...
    /* start at the thread number, iterate by number of threads */
    for(int px=*(int *)thread_num; px<width;  px+=nthreads)
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            /* save colors to be printed later */
...
main( int argc, char **argv )
{
    ...
    /* array to hold thread number */
    int thread_nums[nthreads];
    
    /* Create n threads to embarrassingly parallize trace_pixels function */
    for (int i = 0; i < nthreads; i++)
    {
        thread_nums[i] = i;
        
        if(pthread_create(&threads[i], NULL, trace_pixels, (void *)(thread_nums + i)))
        {
    ...
    /* Print pixels generated by threads */
